...
IH 預設跟隨劇場六段，不再獨立循環。01 紫色待機、02 綠色偵測、03 紅色加熱、04 琥珀色降功率排煙、05 綠色熱能關閉、06 淺綠完成。兩個投影頁面的 PM2.5 使用相同情境曲線；外部數值接管時沿用外部 PM2.5。失去同步超過 4 秒後顯示待機，避免繼續假裝加熱。

本機 IH 透過 8776 的共用服務讀取分鏡。GitHub Pages 請在同一瀏覽器、同一網域分頁開啟 preview.html 與 ih.html；只提供該瀏覽器內同步，不跨裝置。多個預覽分頁各自同步其左右屏，IH 跟隨最近開啟／操作的劇場。校正面板仍可選獨立示範模式。

已驗證兩個預覽可以獨立暫停，以及 IH 在 03 顯示紅色 600 W、06 顯示淺綠色 0 W 且熄環。
